...
# lund weight 12/11/15

##########################################################

# setwd("/Users/choonoo/WNV")

# save.image("~/lund_weight_dec11.RData")

# load("~/lund_weight_dec11.RData")
...
# read in updated data 12/11

##########################################################

read.xls(xls="./Lund_Weight_12_11_15/097 weights 12_10_15.xlsx", sheet=1, skip=2,colClasses=c("NULL",rep(NA,37)), stringsAsFactors=F) -> line_97

read.xls(xls="./Lund_Weight_12_11_15/095 weights 12_10_15.xlsx", sheet=1, skip=2,colClasses=c("NULL",rep(NA,37)),stringsAsFactors=F) -> line_95

read.xls(xls="./Lund_Weight_12_11_15/092 weights 12_10_15.xlsx", sheet=1, skip=2,colClasses=c("NULL",rep(NA,37)),stringsAsFactors=F) -> line_92

read.xls(xls="./Lund_Weight_12_11_15/091 weights 12_10_15.xlsx", sheet=1, skip=2,colClasses=c("NULL",rep(NA,37)),stringsAsFactors=F) -> line_91

read.xls(xls="./Lund_Weight_12_11_15/082 weights 11_5_15.xlsx", sheet=1, skip=2,colClasses=c("NULL",rep(NA,37)),stringsAsFactors=F) -> line_82

read.xls(xls="./Lund_Weight_12_11_15/088 weights 11_18_15.xlsx", sheet=1, skip=2,colClasses=c("NULL",rep(NA,37)),stringsAsFactors=F) -> line_88

read.xls(xls="./Lund_Weight_12_11_15/087 weights 11_18_15.xlsx", sheet=1, skip=2,colClasses=c("NULL",rep(NA,37)),stringsAsFactors=F) -> line_87

# clean score days
days = gsub("d","D",gsub("\\.","",names(line_97)[9:37]))
...
format_data(line_97) -> lund1
format_data(line_95) -> lund2
format_data(line_92) -> lund3
format_data(line_91) -> lund4
format_data(line_82) -> lund5
format_data(line_88) -> lund6
format_data(line_87) -> lund7


# collapse
rbind(lund1, lund2, lund3, lund4, lund5, lund6, lund7) -> new_lund_line
...
flag_weight_drop = sapply(1:dim(new_lund_line_v2)[1],function(x)sum(new_lund_line_v2[x,days_percent] <= -20, na.rm=T) > 0)
...
identical_weights = sapply(1:dim(new_lund_line_v2)[1],function(x)sum(diff(na.omit(as.numeric(as.character(new_lund_line_v2[x,days_percent])))) == 0) > 0)
...
# check weights that changed more than 10% day to day, absolute value of differences
weight_change_flag = sapply(1:dim(new_lund_line_v2)[1],function(x)sum(abs(diff(na.omit(as.numeric(as.character(new_lund_line_v2[x,days_percent]))))) >= 10) > 0)
...
if(length(which(new_lund_line_v3[,"Death_Euthanized"] > 28 | new_lund_line_v3[,"Death_FoundInCage"] > 28)) == 0){
  new_lund_line_v3$Flag_Death_Date = FALSE
...
new_lund_line_v4[,"Death_Date"] <- as.Date(new_lund_line_v4[,"Death_Date"], format="%Y-%m-%d")

# write processed file
write.table(file="./Lund_Weight_12_11_15/Lund_Weight_12_9_15_upload_GC.txt", x=new_lund_line_v4, sep="\t", quote=F, row.names=F)
...
# read in full data
read.xls(xls="./Lund_Weight_12_11_15/Lund_Weight_12_3_15_GC.xlsx",sheet=1, stringsAsFactors=F) -> lund_weight_full

# fix flag per weight change in old data
lund_weight_full$Flag_Per_Day_Weight_Change_v2 = NA

# check weights that changed more than 10% day to day
weight_change_flag = sapply(1:dim(lund_weight_full)[1],function(x)sum(abs(diff(na.omit(as.numeric(as.character(lund_weight_full[x,days_percent]))))) >= 10) > 0)

lund_weight_full$Flag_Per_Day_Weight_Change_v2 = weight_change_flag

lund_weight_full[which(lund_weight_full[,"Flag_Per_Day_Weight_Change"] != lund_weight_full[,"Flag_Per_Day_Weight_Change_v2"]),]

# write to table
write.table(file="./Lund_Weight_12_11_15/Lund_Weight_12_11_15_full_GC.txt", x=lund_weight_full, sep="\t", quote=F, row.names=F)

# remove any lines from full that are in updated data 11/30
remove_lines = intersect(new_lund_line_v4[,"UW_Line"], lund_weight_full[,"UW_Line"])

lund_weight_full[-as.vector(unlist(sapply(remove_lines,function(x)which(x==lund_weight_full[,"UW_Line"])))),] -> lund_weight_full_v2

# update date format
lund_weight_full_v2[,"Date_Infected"] <- as.Date(lund_weight_full_v2[,"Date_Infected"],format="%Y-%m-%d")

# clean 11/30, if no questions, add to full
rbind(lund_weight_merge_v2, new_lund_line_v4) -> lund_weight_full_12_11

# check no duplicates
dim(lund_weight_full_12_11)[1] == length(unique(lund_weight_full_12_11[,"ID"]))

# write processed file, has all corrections
write.table(file="./Lund_Weight_12_11_15/Lund_Weight_12_11_15_full_GC.txt", x=lund_weight_full_12_11, sep="\t", quote=F, row.names=F)
